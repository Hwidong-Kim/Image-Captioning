...
def train(model, data_loader, test_loader, optimizer, tokenizer, epoch, warmup_steps, device, scheduler, config, do_amp=False,
    ...
    for i, (image, caption, object_labels, image_ids, gold_caption) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if i > 0 and i % args.eval_steps == 0:
            ...
            result_file = save_result(vqa_result, args.result_dir, 'cpation_result_%d' % i)
            model.eval()
            if utils.is_main_process():
                ...
                if (result["Bleu_4"] + result["METEOR"])*100 > best_cider:
                    best_cider = (result["Bleu_4"] + result["METEOR"])*100
                    ...
                    torch.save({
                        ...
                        'config': config,
                        'epoch': i,
                    }, os.path.join(args.output_dir, f'checkpoint_best_{epoch}.pth'))
            dist.barrier()
            model.train()
...
def main(args, config):
    ...
    #### Dataset ####
    print("Creating caption datasets")
    ...
    vqa_result = evaluation(model, test_loader, tokenizer, device, config)
    result_file = save_result(vqa_result, args.result_dir, 'caption_result_epoch10')
    if utils.is_main_process():
        result = cal_metric(result_file)
    dist.barrier()
    for epoch in range(start_epoch, max_epoch):
        ...
        if not args.evaluate:
            ...
            train_stats = train(model, train_loader, test_loader, optimizer, tokenizer, epoch, warmup_steps, device, lr_scheduler,
        ...
        vqa_result = evaluation(model, test_loader, tokenizer, device, config)
        result_file = save_result(vqa_result, args.result_dir, 'caption_result_epoch%d' % epoch)
        if utils.is_main_process():
            result = cal_metric(result_file)
...
#/home/hdd2/hdkim/coco_caption_base_$lr \
...
if __name__ == '__main__':
    ...
    parser.add_argument('--eval_steps', default=3500, type=int)
